    ...
    <div>
      ...
      <main className="mt-4 md:mt-6 lg:mt-10 px-4 md:px-8 lg:px-16 xl:px-20">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold dark:text-gray-200">Olá, {user?.user_metadata.name.split(' ')[0] || "Micro Herói"}!</h1>
            <p className="text-lg md:text-xl text-gray-600 dark:text-gray-300">Acompanhe sua jornada financeira</p>
          </div>
        </header>

        <section className="flex flex-col lg:flex-row w-full justify-center items-center gap-6" aria-labelledby="dashboard-heading">
          <div className="flex flex-col w-full lg:w-1/2 gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <article className="bg-white rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-purple-500 w-full dark:bg-gray-800 dark:text-gray-200">
                <h3 className="text-gray-500 text-sm font-medium dark:text-gray-300">Receita Total</h3>
                <p className="text-xl md:text-2xl font-semibold text-gray-800 mt-2 dark:text-gray-200">{formatCurrency(metrics.receita)}</p>
              ...
              <article className="bg-white rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-blue-500 w-full dark:bg-gray-800 dark:text-gray-200">
                <h3 className="text-gray-500 text-sm font-medium dark:text-gray-300">Despesas</h3>
                <p className="text-xl md:text-2xl font-semibold text-gray-800 mt-2 dark:text-gray-200">{formatCurrency(metrics.despesa)}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <article className="bg-white rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-green-500 w-full dark:bg-gray-800 dark:text-gray-200">
                <h3 className="text-gray-500 text-sm font-medium dark:text-gray-300">Lucro</h3>
                <p className="text-xl md:text-2xl font-semibold text-gray-800 mt-2 dark:text-gray-200">{formatCurrency(metrics.lucro)}</p>
              ...
              <article className="bg-white rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-yellow-500 w-full dark:bg-gray-800 dark:text-gray-200">
                <h3 className="text-gray-500 text-sm font-medium dark:text-gray-300">Margem</h3>
                <p className="text-xl md:text-2xl font-semibold text-gray-800 mt-2 dark:text-gray-200">{metrics.margem.toFixed(1)}%</p>
        ...
        <section className="flex flex-col lg:flex-row w-full justify-center items-start gap-6 mt-20 mb-20">
          ...
          <div className="w-full lg:w-1/2 mt-6 lg:mt-0">
            <div className="flex justify-between mb-4">
              <h3 className="text-xl md:text-2xl font-bold text-gray-800 dark:text-gray-200">Últimas Transações</h3>
              <a href="/transactions" className="text-sm text-purple-600 hover:bg-gray-200 p-2 rounded-md transition duration-300 dark:hover:bg-gray-800">Ver todas</a>
            </div>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-600 rounded-xl">
                  {recentTransactions.map((tx, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-200">{tx.date}</td>
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-200">{tx.description}</td>
                      <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{tx.category?.name || 'Nenhuma'}</td>
